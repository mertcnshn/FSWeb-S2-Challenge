...
function cumlelereDonustur(sentences, seperator = ",") {
  return sentences.map((sentence) => sentence.join(seperator));
}
...
function paragrafOlustur(cumlelerArr, callback, callbackFn) {
  const newArr = callbackFn(cumlelerArr, " ");
  const cumlelerDizisi = newArr.filter((newArr, index) => {
...
newArr[9]
  ...
  */
  return callback(...cumlelerDizisi);
}
